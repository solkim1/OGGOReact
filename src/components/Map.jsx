...
const Map = ({ locations, center }) => {
  const initializeMap = useCallback(() => {
    ...
    const searchPubTransPathAJAX = async (sx, sy, ex, ey, index) => {
      ...
      if (cachedData && cachedData.result && cachedData.result.path && cachedData.result.path.length > 0) {
        ...
        const url = `https://api.odsay.com/v1/api/searchPubTransPathT?SX=${sx}&SY=${sy}&EX=${ex}&EY=${ey}&apiKey=2r2QB8AHuKaddIjuRbbjOA.....123123`;
    ...
    const callMapObjApiAJAX = async (mapObj, sx, sy, ex, ey, index, isCached) => {
      ...
      if (cachedData && isCached) {
        ...
        const url = `https://api.odsay.com/v1/api/loadLane?mapObject=0:0@${mapObj}&apiKey=2r2QB8AHuKaddIjuRbbjOA.....123123`;
  ...
  useEffect(() => {
    ...
    script.src = `https://oapi.map.naver.com/openapi/v3/maps.js?ncpClientId=h7fnyo8jb3.....123123`;
