...
const ScheduleMapPage = () => {
  ...
  const {
    ...
    startDate,
    endDate,
    scheNum // AllSchedules에서 전달된 일정 번호
  } = location.state || {};
  ...
  const fetchSchedule = useCallback(async (start, end) => {
    try {
      ...
      console.log("TEST",{userId,
        ...
        endDate,
        scheNum});
      if (scheNum) { // 일정 번호가 있을 경우
        const response = await fetch(`/plan/api/schedules/patchschedule?scheNum=${scheNum}`);
        if (!response.ok) {
